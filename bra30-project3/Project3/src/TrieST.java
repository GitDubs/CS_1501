...
public class TrieST<Value> {
    private static final int R = 33;        // Possible Characters in a VIN
   ...
   /**
    ...
    private Node get(Node x, String key, int d) {
        ...
        char c = key.charAt(d);
        return get(x.next[vinCharacterToInt(c)], key, d+1);
    }

    private int vinCharacterToInt(char c){
        if (c >= '0' && c <= '9'){
            return c - 48;
        }else if(c >= 'A' && c <= 'H'){
            return (c - 55);
    ...
    private Node put(Node x, String key, Value val, int d) {
        ...
        char c = key.charAt(d);
        x.next[vinCharacterToInt(c)] = put(x.next[vinCharacterToInt(c)], key, val, d+1);
        return x;
    }
